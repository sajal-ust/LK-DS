...
def run_evaluation(recommendation_output_file, test_data_file="test_data.csv"):
    """
    Evaluate Top-K scheme/product recommendations using precision, recall, and F1.

    Args:
        recommendation_output_file (str): Path to CSV with Partner_id, Recommended_Products.
        test_data_file (str): Path to test data CSV with actual product purchases.

    Returns:
        pd.DataFrame: Top-K evaluation results.
    """

    recommendations_df = pd.read_csv(recommendation_output_file)
    test_df = pd.read_csv(test_data_file)

    # Rename Partner_ID to Partner_id if needed
    recommendations_df = recommendations_df.rename(columns={"Partner_ID": "Partner_id"})

    # Identify product columns (all except metadata)
    meta_cols = [
    ...
    # Create actual purchased product lists
    ...
    # Convert stringified product list back to list
...
if __name__ == "__main__":
    eval_result = run_evaluation("User_Based_Recommendations.csv", "test_data.csv")
    print(eval_result)
    eval_result.to_csv("Evaluation_Metrics.csv", index=False)
